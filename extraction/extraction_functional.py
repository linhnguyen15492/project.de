...
def source_data_from_parquet(parquet_file_name):
    try:
        ...
        logger.info(
            f"{parquet_file_name} : extracted {df_parquet.shape[0]} records from the parguet file"
        )
    except Exception as e:
        logger.exception(
            f"{parquet_file_name} : - exception {e} encountered while extracting the parguet file"
        )
...
def source_data_from_csv(csv_file_name):
    try:
        ...
        logger.info(
            f"{csv_file_name} : extracted {df_csv.shape[0]} records from the csv file"
        )
    except Exception as e:
        logger.exception(
            f"{csv_file_name} : - exception {e} encountered while extracting the csv_file_name file"
        )
        df_csv = pd.DataFrame()
    return df_csv
...
def source_data_from_api(api_endpoint):
    try:
        ...
        if apt_status == 200:
            ...
            logger.info(
                f"{apt_status}- extracted {df_api.shape[0]} records from the csv file"
            )
        else:
        ...
        logger.exception(
            f"{apt_status} : - exception {e} encountered while reading data from the api"
        )
        df_api = pd.DataFrame()
    return df_api
...
def source_data_from_table(db_name, table_name):
    try:
        ...
        with sqlite3.connect(db_name) as conn:
            ...
            logger.info(
                f"{db_name}- read {df_table.shape[0]} records from the table: {table_name}"
            )
    except Exception as e:
        logger.exception(
            f"{db_name} : - exception {e} encountered while reading data from the table: {table_name}"
        )
...
def source_data_from_webpage(web_page_url, matching_keyword):
    try:
        ...
        df_html = df_html[0]
        logger.info(
            f"{web_page_url}- read {df_html.shape[0]} records from the page: {web_page_url}"
        )
    except Exception as e:
        logger.exception(
